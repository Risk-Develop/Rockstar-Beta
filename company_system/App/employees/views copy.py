...
from App.users.models import Staff, Role
from .forms import StaffForm, EmployeeForm
from django.http import JsonResponse


def dashboard_user_list(request):
    qs = Staff.objects.all().order_by('last_name', 'first_name')
    return render(request, 'dashboard/d_user_list.html', {'staff_list': qs})
...
def dashboard_user_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('dashboard_user_mgnt')  # redirect to dashboard user management
    else:
        form = StaffForm()
    return render(request, 'dashboard/d_user_form.html', {'form': form, 'action': 'Add'})
...
def dashboard_user_edit(request, pk):
    ...
    return render(request, 'dashboard/d_user_form.html', {'form': form, 'action': 'Edit', 'staff': obj})

def dashboard_user_detail(request, pk):
    obj = get_object_or_404(Staff, pk=pk)
    return render(request, 'dashboard/d_user_detail.html', {'staff': obj})
