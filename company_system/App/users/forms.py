...
class StaffForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            # Address fields widgets
            'street_address': forms.TextInput(attrs={'class': 'input border rounded px-2 py-1 w-full', 'placeholder': 'Enter street address'}),
            'city': forms.TextInput(attrs={'class': 'input border rounded px-2 py-1 w-full', 'placeholder': 'Enter city'}),
            'province': forms.TextInput(attrs={'class': 'input border rounded px-2 py-1 w-full', 'placeholder': 'Enter province'}),
            'postal_code': forms.TextInput(attrs={'class': 'input border rounded px-2 py-1 w-full', 'placeholder': 'Enter postal code'}),
            'country': forms.TextInput(attrs={'class': 'input border rounded px-2 py-1 w-full', 'placeholder': 'Enter country'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Make employee_number editable for new employees (Add mode)
        # For Edit mode, the template handles readonly via hidden input
        if not self.instance.pk:
            self.fields['employee_number'].widget.attrs.pop('readonly', None)
