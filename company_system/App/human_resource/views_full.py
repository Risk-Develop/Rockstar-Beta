...
from App.users.models import Staff, Role  # adjust if models are elsewhere
from App.users.forms import StaffForm     # if using a form for add/edit
...
def attendance_clock(request):
    ...
    if not shift_rule or not (shift_rule.clock_in_start and shift_rule.clock_out):
...
def hr_shift_rules_list(request):
    ...
    if request.method == 'POST':
        for rule in rules:
            ...
            if clock_in_start and clock_out:
                try:
